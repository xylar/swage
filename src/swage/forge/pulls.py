...
`labeled` event whose label is `automerge` (design-v1.md 2.2), so a pull request
...
(design-v1.md 5.2). So conda-forge merges, and swage's job is to arm it.

Nothing here decides whether to arm anything. That is the trust ladder
(design-v1.md 5.4), and it lives in the command.
"""
...
def arm_automerge(github: GitHub, pull: BotPullRequest) -> None:
    ...
    (design-v1.md 2.2, 5.5): labeling first guarantees the label is stripped,
