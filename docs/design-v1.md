...
!!! note "Frozen at v1.0.0"
    ...
    which cites this file by section (`v1 §3.3.7`). It is not edited: a
    decision made during v2 goes in `DESIGN.md` §16 and the commit that
    acts on it.
...
**A third value says there is nothing to read at all.** `source: none` records
that this feedstock packages no python distribution, and swage then plans it no
further and reports it as `NOT RECONCILED` — the front section's case,
...
> a feedstock nobody finished configuring — and the entry stops swage
> reconciling one, which is too much to record without a sentence.
>
> **Inference was the alternative and was not taken.** swage could compare the
...
**The comment on a migration is its own, and it ends by asking conda-forge
for a rerender.** The first conversion swage pushed got the ordinary refusal
comment, which said `recipe/recipe.yaml` had been updated to match the
release and nothing about that file having been created by a conversion an
hour earlier — and the pull request then could not build until its
maintainer worked out why. The CI configuration under `.ci_support/` and
`.scripts/` is generated by conda-smithy from the recipe and
`conda-forge.yml`, never written, and a conversion changes both; nothing
regenerates it until somebody asks. So the comment says what the two commits
are, that the label would not have been added whatever the checks found (the
ceiling above, rather than the findings, is the reason), and closes with the
line conda-forge's webservice answers by pushing a rerender to the pull
request, spelled exactly as conda-forge documents it:
`@conda-forge-admin, please rerender`. The sentence above it says why, since
to the maintainer it reads as swage asking a bot to push to their pull
request.

That commit lands after swage's, which on an ordinary update would strip an
`automerge` label (§2). On a migration there is no label to strip, and an
ordinary update never asks for one: a dependency change touches nothing the
CI configuration is generated from.
