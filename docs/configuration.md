...
|---|---|
| `<line>` is in `<package>`'s `<section>` requirements and in no upstream version | [`add_requirements`](config/names.md#add_requirements) to keep the line, [`retire`](config/names.md#retire) to delete it — or [`name_map`](config/names.md#name_map), where the diff shows swage adding the same package under another name |
| `<line>` in `<package>`'s `<section>` requirements is upstream's name for what conda-forge publishes as `<other>` | [`name_map`](config/names.md#name_map) |
