...
| `esmf` | a Fortran and C++ library, in `nompi`, `openmpi` and `mpich` variants | the first feedstock with a reader of its own (DESIGN.md 3.6.5). It carries upstream's files rather than only the recipe, because what has to keep working is the *reading* of them |
...
| File | What it is |
|---|---|
| `common.mk` | `build/common.mk` from the ESMF 8.9.1 tarball, unedited. 4,640 lines, of which the reader looks at the eleven `ESMF_<toggle>_LIBS` assignments — vendored whole because the thing most likely to break the reader is ESMF restructuring this file, and only the real one catches that |
| `build.sh` | `recipe/build.sh` off the feedstock's default branch, which says which of those toggles are on |
| `configure.ac` | the vendored ParallelIO's, which states the version ESMF builds against |
| `recipe.yaml` | the recipe those two are reconciled against |
